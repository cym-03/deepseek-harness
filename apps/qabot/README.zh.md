...
| `GET /v1/analytics/overview?from&to&group` | 返回按角色隔离的指标、归因列表和等长上期对比 |
| `POST /v1/analytics/voc` | 返回或缓存按角色隔离的员工声音摘要 |
| `GET /v1/faqs/recommendations` | 返回近 30 天最常用五条和各组已启用推荐问题 |
| `GET/POST/PATCH/DELETE /v1/operations/faqs` | 维护直答问题、文档链接和存入 MySQL 的图片附件 |
| `GET /v1/system/audit?page&pageSize=10&from&to` | SystemAdmin 服务端分页查询可读的统一操作审计，每页最多十条 |
...
| `QABOT_VOC_MODEL` | `DSH_QABOT_MODEL` 或 `deepseek-chat` | 生成并缓存员工声音摘要的文本模型 |
...
- **运营分析**：运营数据只以 MySQL 为权威来源。每个有效员工问题都会记录领域、可靠知识命中或常见问题命中；AI 建议转人工与员工实际选择分开统计，结束工单保存真实 `resolved_at`，迁移的历史时间明确标记为估算。主管可查看全部分组，人事、行政、IT、财务专员由服务端强制限定在本组。每组最多启用五条常见问题，限制由事务保证；问题完全一致或高置信且明显领先第二候选时，系统直接返回已配置的答案、图片和链接，不调用模型。员工声音摘要只向文本模型发送汇总指标和归因数据，相同数据复用 MySQL 缓存，模型不可用时返回规则摘要。统一审计返回中文操作、可读内容和员工姓名，并固定每页最多十条。
