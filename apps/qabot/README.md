...
| `GET /v1/analytics/overview?from&to&group` | Return role-scoped metrics, attribution lists, and equal-length previous-period comparisons |
| `POST /v1/analytics/voc` | Return or cache a role-scoped employee-voice summary |
| `GET /v1/faqs/recommendations` | Return the five most-used recommendations and enabled questions by group |
| `GET/POST/PATCH/DELETE /v1/operations/faqs` | Maintain direct-answer questions, links, and MySQL-backed image attachments |
| `GET /v1/system/audit?page&pageSize=10&from&to` | Let SystemAdmin query readable unified audit records with server-side pagination |
...
| `QABOT_VOC_MODEL` | `DSH_QABOT_MODEL` or `deepseek-chat` | Text model used for cached employee-voice summaries |
...
Operations analytics uses MySQL as its sole authority. Each effective employee turn records its assigned domain and reliable knowledge or configured-question match. Actual handoff selection is separate from an AI suggestion, and completed tickets store a true `resolved_at`; migrated historical values remain marked as estimates. Managers can query all groups, while HR, administration, IT, and finance operators are forcibly scoped to their own group. Enabled configured questions are capped at five per group in a transaction. Exact or clearly dominant high-confidence matches return the stored answer, images, and links without calling a model. Employee-voice generation sends only aggregate metrics and attribution data to the text model, reuses a MySQL cache for unchanged data, and returns a rule summary when the model is unavailable. Unified audit results expose readable operations and employee names, ten rows per page.
...
- **Operations analytics:** MySQL is the authority for operations data. Each substantive employee question records its service group and reliable knowledge or configured-answer match; AI handoff suggestions remain distinct from employee-selected transfers. Supervisors can view every group, while Human Resources, Administration, IT, and Finance specialists are restricted to their own group by the server. Configured answers, employee-voice summaries, readable audit records, and ten-row result pages use the same role-scoped data.
