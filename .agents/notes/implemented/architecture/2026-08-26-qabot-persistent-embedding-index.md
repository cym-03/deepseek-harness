...
Repeated startup with unchanged documents and model performs no embedding calls. Remote-model document updates recalculate only changed chunks, while a model change performs one deliberate full refresh. Local TF-IDF remains correct at the cost of a full refresh after any corpus change. Existing databases perform a one-time refresh of legacy rows. Configured visual ingestion downloads Feishu document images, retains their bytes for model changes, prunes images removed from the source document, and supports cross-modal text-to-image retrieval without exposing visual vectors to text cosine search. Repeated identical text-to-image queries reuse the persisted query vector, and returned images survive refresh through message-linked references. Visual ingestion requires Feishu media-download permission and a DashScope API key; without `VISION_EMBED_MODEL` it remains inactive.
