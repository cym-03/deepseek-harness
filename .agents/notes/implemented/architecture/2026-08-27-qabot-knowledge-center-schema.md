...
MySQL migrations 2 through 4 introduce separate knowledge sources, documents, document versions, applicable departments, chunks, assets, embeddings, reviews, synchronization jobs, an explicit document publication status, and durable image bytes. Document versions use the fixed lifecycle `draft`, `pending_review`, `published`, `archived`, and `rejected`. Content and asset hashes are durable fields. Text and vision embeddings record their target type, modality, model key, dimensions, and source-content hash, so vectors from different models or modalities cannot be treated as interchangeable.
...
The employee service desk publishes only company-wide knowledge, so department applicability is not evaluated by the current retrieval provider. Restricted information uses a separate Feishu private-chat application. With the MySQL backend, startup and every knowledge mutation project documents, review versions, chunks, image assets, and already-computed vectors into `hr_system` in one transaction. Projection reads stored vectors and never invokes an embedding provider. `kb.db` remains the active FTS and similarity-search cache until the MySQL repository becomes authoritative.
...
The `hr_system` database has nine governed-knowledge tables plus document publication fields and image bytes. Every table and column has a Chinese comment. Qabot enforces review, publication status, and validity windows before synchronized content affects employee answers. Visual assets stay on their last published data when a changed document is staged, so synchronization does not replace returned images before text review completes. MySQL now holds a durable operational copy of the complete knowledge index; a later provider cutover will rebuild the SQLite retrieval cache from MySQL and make MySQL the single source of truth.
